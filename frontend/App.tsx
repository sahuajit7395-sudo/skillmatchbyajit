...
const AdminLoginScreen: React.FC<AdminLoginProps> = ({ navigation }) => {
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : 'Invalid admin credentials';
...
const AdminDashboardScreen: React.FC<AdminDashboardProps> = () => {
  const { events, volunteers, isLoading, addEvent, assignTask, updateEventStatus } =
    useEventContext();
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      await addEvent(title.trim(), description.trim(), status);
      setTitle('');
      setDescription('');
      setStatus('Upcoming');
    } catch {
      // Error surfaced in EventContext
    }
  };
  ...
  const handleAssignTask = async () => {
    ...
    try {
      await assignTask(selectedEventForAssign, selectedVolunteer);
    } catch {
      // Error surfaced in EventContext
    }
  };
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await updateEventStatus(selectedEventForStatus, newStatus);
    } catch {
      // Error surfaced in EventContext
    }
  };
